...
- **Semantics ve CustomSemanticsAction Hatası:** Eğer widget'larda semantik erişilebilirlik (ekran okuyucu vb.) için `customSemanticsActions` altında `CustomSemanticsAction` kullanıyorsanız ve derleme sırasında "The method 'CustomSemanticsAction' isn't defined" hatası alıyorsanız, o dosyanın başına mutlaka `import 'package:flutter/semantics.dart';` eklemelisiniz. Bu sınıf varsayılan `material.dart` paketiyle doğrudan çekilemeyebilir.
